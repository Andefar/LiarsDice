...
First step is to create a possible worlds model $M$, where $\omega$ is the set of all states within that world\todo{within the model?}. A state in the possible world model represents a set of strategies for each of the players in the game.
...
This fact of common knowledge of rationality is a quite rare scenario in practice\todo{only when \emph{humans} play actual games, right? something about games would not be "fun to play" if every move is given beforehand}. Often players are not rational at all or at least it is not common knowledge. In these cases it would be irrational to choose a strategy based on the result of the backwards induction, because a player do not know the reasons for his/hers opponents choices.